...
///
/// Controls timeout and polling behavior when waiting for captcha solutions.
///
/// # Example
///
/// ```rust
...
pub struct CaptchaSolverServiceConfig {
    /// Maximum time to wait for captcha solution before timing out.
...
impl CaptchaSolverServiceConfig {
    /// Create a new builder for CaptchaSolverServiceConfig.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// ```
    pub fn builder() -> CaptchaSolverServiceConfigBuilder {
        CaptchaSolverServiceConfigBuilder::default()
    }

    /// Fast configuration preset.
    ///
    /// Uses a shorter timeout (60s) and aggressive polling (2s).
    ...
    /// Good for slow providers or complex captchas.
    ///
...
///
/// Provides a fluent API for configuring the captcha service.
#[derive(Debug, Clone)]
...
impl Default for CaptchaSolverServiceConfigBuilder {
    fn default() -> Self {
        let config = CaptchaSolverServiceConfig::balanced();
        Self {
...
impl CaptchaSolverServiceConfigBuilder {
    ...
    /// Build the CaptchaSolverServiceConfig.
    ///
    ...
    /// Build and validate the CaptchaSolverServiceConfig.
    ///
    ...
    /// let result = CaptchaSolverServiceConfig::builder()
    ///     .timeout(Duration::from_secs(10))
...
mod tests {
    ...
    fn test_config_builder_default() {
        let config = CaptchaSolverServiceConfigBuilder::new().build();
        assert_eq!(config.timeout, Duration::from_secs(120));
        assert_eq!(config.poll_interval, Duration::from_secs(3));
    }
